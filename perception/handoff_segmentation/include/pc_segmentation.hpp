...
// Defined by Priyanka - Testing out object detection without plane segmentation
...
//#endif
 ...
 // void pcl::compute3DCentroid(pcl::PointCloud<PointT> &cluster, Eigen::Vector4f centroid);
  ...
  int 
  getClosestObject(std::map<size_t, pcl::PointCloud<PointT> > clusterids_to_clusters, size_t &closestObjectIndex);
  
  void
  checkIfRedBottleExists(std::map<size_t, std::pair<double,double> > cluster_to_avg_hue_sat, bool &ifRedBottleExists);
  
  // Delete this method after the handoff demo
  int
  getClosestRedObject(std::map<size_t, pcl::PointCloud<PointT> > clusterids_to_clusters, 
					   std::map<size_t, std::pair<double,double> > cluster_to_avg_hue_sat, 
					   size_t &closestObjectIndex);
  
  bool 
  checkIfBetweenMergeDistance(pcl::PointCloud<PointT> cluster1, pcl::PointCloud<PointT>  cluster2);

  // void --> Change it back to void later on
  bool      
  ...
  void
  computeAvgHueSatForClusters(
         std::map<std::size_t, std::pair<double, double> > &cluster_to_avg_hue,
         pcl::PointCloud<PointT>::CloudVectorType &clusters,
  ...
  // int  --> Change back to int when the demo is done
  std::pair<int,bool>      // Delete this later on
