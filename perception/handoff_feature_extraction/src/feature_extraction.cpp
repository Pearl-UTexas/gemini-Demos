...
#include <handoff_feature_extraction/Features.h>
#include <handoff_feature_extraction/FeaturesArray.h>
...
main (int argc, char **argv)
{
  ...
  //pub = nh->advertise<handoff_segmentation::Features>(outRostopic,5);
  pub = nh->advertise<handoff_feature_extraction::FeaturesArray>(outRostopic,5);
 ...
 // planeSub = nh->subscribe(planeRostopic, 1, plane_cb);
  ...
  while (!interrupt && (!pA.viz || !viewer->wasStopped ()))
  {
    ...
    if(gotCluster){
        ...
        // Display the bounding boxes for the object
        //multi_plane_app.displayOrientedBoundingBox(clusters);
    }
    ...
    handoff_feature_extraction::FeaturesArray rosMsg;
    rosMsg.header.stamp = ros::Time::now();

    for(int i = 0; i < feats.size(); i++) {
      handoff_feature_extraction::Features ft;
