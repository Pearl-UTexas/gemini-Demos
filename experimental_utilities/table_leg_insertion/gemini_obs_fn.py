...
from task3.srv import object_frame_data
...
class WayPoints:
	...
	def __init__(self, z_init): #, x_init):
		...
		self.n_waypoints = 5
		self.waypts = np.zeros((self.n_waypoints,self.nState))
	...
	def jointTraj_waypoint(self):
		# self.waypts[0,:] = array([-1.165, 1.892, 1.962, 0.044, 1.979, -0.566])
		# self.waypts[1,:] = array([-1.175, 1.901, 1.962, 0.044, 1.979, -0.566])
		# self.waypts[2,:] = array([-1.275, 2.152, 2.255, 0.044, 1.978, -0.566])
		# self.waypts[3,:] = array([-1.65, 2.569, 2.783, 0.35, 2.038, -0.564])
		# self.waypts[4,:] = array([-1.724, 2.591, 2.748, 0.262, 2.08, -0.561])
		# self.waypts[5,:] = array([-1.321, 2.529, 2.697, -0.385, 2.08, -0.5617])
		# self.waypts[6,:] = array([-1.08, 2.571, 2.708, -0.638, 2.08, -0.561])
		# self.waypts[7,:] = array([-1.36, 2.605, 2.71, -0.273, 2.087, -0.561])
		# self.waypts[8,:] = array([-1.523, 2.451, 2.429, -0.271, 2.091, -0.561])
		# self.waypts[9,:] = array([-1.291, 2.143, 1.947, -0.538, 1.873, -0.561])
		# self.waypts[10,:] = array([-1.692, 2.256, 2.066, -0.384, 1.675, -0.561])
		# self.waypts[11,:] = array([-1.683, 2.254, 2.066, -0.384, 1.676, 0.008])
		# self.waypts[12,:] = array([-1.685, 2.248, 2.066, -0.384, 1.677, 0.744])
		# self.waypts[13,:] = array([-1.683, 2.221, 2.066, -0.384, 1.677, 1.462])
		# self.waypts[14,:] = array([-1.683, 2.221, 2.066, -0.384, 1.677, 3.462])
		# self.waypts[15,:] = array([-1.683, 2.221, 2.066, -0.384, 1.677, 5.462])
		# self.waypts[16,:] = array([-1.683, 2.221, 2.066, -0.384, 1.677, 10.462])
		# self.waypts[17,:] = array([-1.683, 2.221, 2.066, -0.384, 1.677, 15.462])

		self.waypts[0,:] = array([-1.683, 2.221, 2.066, -0.384, 1.677, 1.462])
		self.waypts[1,:] = array([-1.683, 2.221, 2.066, -0.384, 1.677, 3.462])
		self.waypts[2,:] = array([-1.683, 2.221, 2.066, -0.384, 1.677, 5.462])
		self.waypts[3,:] = array([-1.683, 2.221, 2.066, -0.384, 1.677, 10.462])
		self.waypts[4,:] = array([-1.683, 2.221, 2.066, -0.384, 1.677, 15.462])
...
class TagTracking():
	...
	def angle_correction(self, cur_ang, prev_ang):
		cur_ang = list(cur_ang)
		for i in range(len(cur_ang)):
			if (prev_ang[i] >= 0.):
				ang_range = floor(prev_ang[i]/pi)
				rem = prev_ang[i]%pi
				if ((cur_ang[i]< prev_ang[i]) and rem > 2.5): # Random limit to get a value of corrected angle > pi 
					print "Doing positive correction"
					cur_ang[i] = cur_ang[i] + (ang_range + 1.)*pi

			elif(prev_ang[i] < 0.):
				ang_range =  ceil(prev_ang[i]/pi)
				rem = prev_ang[i] - ang_range
				# print "ang_range = ", ang_range
				if ((cur_ang[i] > prev_ang[i]) and abs(rem) > 2.5): # Random limit to get a value of corrected angle > pi 
					print "Doing negaive correction"	
					cur_ang[i] = cur_ang[i] + (ang_range - 1.)*pi

		self.correctAngle = False
		return tuple(cur_ang)
...
# >>>>>>> Stashed changes
	...
	planner.jointTraj_waypoint()
	nPts = 0
	real_object_position = zeros(6)
	obs_error = zeros((planner.n_waypoints, planner.nState))
	...
	while (not(rospy.is_shutdown()) and nPts < planner.n_waypoints):
		...
		jointTarg = planner.waypts[nPts,:].tolist()
